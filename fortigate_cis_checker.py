...
class FortigateCISAudit:
    ...
    def grep_config(self, pattern):
        """Grep config hỗ trợ tìm theo từ khóa, không cần đúng thứ tự."""
        try:
            # Tìm từng dòng trong config_content
            for line in self.config_content.splitlines():
                words = line.lower().split()
                # Nếu tất cả các từ trong pattern có mặt trong line
                if all(token in words for token in pattern.lower().split()):
                    return True
            return False
        except Exception as e:
    ...
    #def grep_config(self, pattern):
    #    """Simulates grep functionality for config file (safe word matching)."""
    #    try:
    #        # Nếu pattern là text đơn giản, tự động thêm word boundary
    #        if not re.search(r"[\\\^\$\.\*\+\?\(\)\[\]\{\}\|]", pattern):
    #            # Thêm \b vào trước/sau mỗi cụm từ để tránh match nhầm
    #            pattern = re.sub(r"(\w+)", r"\\b\1\\b", pattern)
    #
    #       return bool(re.search(pattern, self.config_content))
    #    except Exception as e:
    #        print(f"Error searching config file: {e}")
    ...
    def check_ips_signatures(self):
        ...
        if (self.grep_config(r"config ips global") and 
            self.grep_config(r"set database")):
    ...
    def check_application_control(self):
        ...
        if (self.grep_config(r"config application list") and 
            self.grep_config(r"set deep-app-inspection disable")):
    ...
    def check_admin_https_redirect(self):
        ...
        if (self.grep_config(r"config system global") and 
            self.grep_config(r"set admin-https-redirect disable")):
    ...
    def check_admin_ssh_grace_time(self):
        ...
        if (self.grep_config(r"config system global") and 
            self.grep_config(r"set admin-ssh-grace-time")):
    ...
    def check_admin_timeout(self):
        ...
        if (self.grep_config(r"config system global") and 
            self.grep_config(r"set admintimeout 4")):
            return "PASS: Admin timeout is set to 4 minutes or less"
    ...
    def check_admin_concurrent_sessions(self):
        ...
        if (self.grep_config(r"config system global") and 
            self.grep_config(r"set admin-concurrent disable")):
            return "FAIL: Concurrent admin sessions not properly limited"
        return "PASS: Concurrent admin sessions enable"
    ...
    def check_strong_encryption(self):
        ...
        if (self.grep_config(r"config system global") and 
            self.grep_config(r"set strong-crypto disable")):
